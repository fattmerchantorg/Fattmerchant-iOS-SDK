...
class TakeTapPayment {
  ...
  func start(completion: @escaping (Transaction) -> Void, failure: @escaping (OmniException) -> Void) {
    // First check if Tap to Pay is available
    guard tapDriver.isAvailable else {
      failure(TakeTapPaymentException.tapToPayNotAvailable)
      return
    }
    ...
    self.getOrCreateInvoice(failure) { (createdInvoice) in
      self.takeTapToPayPayment(
        ...
        let voidAndFail: (OmniException) -> Void = { exception in
          ...
          self.tapDriver.void(transactionResult: tapToPayResult) { _ in
            failure(exception)
          }
        }
          
        self.createCustomer(tapToPayResult, voidAndFail) { (createdCustomer) in
          self.createPaymentMethod(for: createdCustomer, tapToPayResult, voidAndFail) { (createdPaymentMethod) in
            self.updateInvoice(createdInvoice, with: createdPaymentMethod, and: createdCustomer, voidAndFail) { (updatedInvoice) in
              self.createTransaction(
                ...
                guard let transactionId = completedTransaction.id else {
                  voidAndFail(TakeTapPaymentException.couldNotCreateTransaction(detail: nil))
                  return
                }
                ...
                self.tapDriver.capture(transaction: completedTransaction) { (success) in
                  if success {
                    completion(completedTransaction)
                  } else {
                    ...
                    self.transactionRepository.update(model: failedTransaction, id: transactionId, completion: { _ in
                      voidAndFail(TakeTapPaymentException.couldNotCaptureTransaction)
                      return
                    }, error: { _ in
                      voidAndFail(TakeTapPaymentException.couldNotCaptureTransaction)
                      return
  ...
  internal func createTransaction(result: TransactionResult,
    ...
    // Set transaction refundability based on tap to pay capabilities
    if !tapDriver.isRefundSupported {
  ...
  private func takeTapToPayPayment(signatureProvider: SignatureProviding?,
                                  transactionUpdateDelegate: TransactionUpdateDelegate?,
                                  userNotificationDelegate: UserNotificationDelegate?,
                                  _ failure: @escaping (OmniException) -> Void,
    ...
    tapDriver.startPaymentSession(with: self.request,
  ...
  fileprivate func createCustomer(_ transactionResult: TransactionResult, _ failure: @escaping (OmniException) -> Void, _ completion: @escaping (Customer) -> Void) {
    var customerToCreate = Customer(firstName: "Tap to Pay", lastName: "Customer")

    // For tap to pay, we may get customer details from the card
    if let firstName = transactionResult.cardHolderFirstName, !firstName.isEmpty {
      customerToCreate.firstname = firstName
    }
    
    if let lastName = transactionResult.cardHolderLastName, !lastName.isEmpty {
    ...
    if let transactionSource = transactionResult.transactionSource,
       transactionSource.lowercased().contains("contactless") || transactionSource.lowercased().contains("tap") {
      if customerToCreate.firstname == "Tap to Pay" {
    ...
    if let invoiceId = request.invoiceId {
      ...
      invoiceRepository.create(model: invoiceToCreate, completion: { createdInvoice in
        guard createdInvoice.id?.isEmpty != true else {
          return failure(TakeTapPaymentException.couldNotCreateInvoice(detail: nil))
        }
