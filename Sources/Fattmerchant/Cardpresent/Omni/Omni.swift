...
public class Omni: NSObject {
    ...
    public weak var mobileReaderConnectionUpdateDelegate:
        MobileReaderConnectionStatusDelegate?

    public weak var tapConnectionUpdateDelegate: TapConnectionStatusDelegate?
    ...
    @available(
        *,
        deprecated,
        message: "Deprecated in favor of `InitializationArgs`."
    )
    ...
    fileprivate func initNewRepos(httpClient: StaxHttpClientProtocol) {
        ...
        staxInvoiceRepository = StaxInvoiceRepositoryImpl(
            httpClient: httpClient
        )
        staxCatalogRepository = StaxCatalogRepositoryImpl(
            httpClient: httpClient
        )
        staxCustomerRepository = StaxCustomerRepositoryImpl(
            httpClient: httpClient
        )
        staxPaymentMethodRepository = StaxPaymentMethodRepositoryImpl(
            httpClient: httpClient
        )
    }
    ...
    public func initialize(
        ...
        Task {
            ...
            guard (try await getStaxSelf(apiKey)) != nil else {
            ...
            #else
                let initArgs = ChipDnaInitializationArgs(
    ...
    @available(
        *,
        deprecated,
        message:
            "Deprecated in favor of the `initialize` with `InitializationArgs`."
    )
    public func initialize(
        params: InitParams,
        test: Bool = false,
    ...
    @available(
        *,
        deprecated,
        message: "Deprecated in favor of the `tokenizeBankAccount` function."
    )
    public func tokenize(
        _ bankAccount: BankAccount,
        _ completion: @escaping (PaymentMethod) -> Void,
        error: @escaping (OmniException) -> Void
    ) {
    ...
    /// - Parameter error: A `(OmniException) -> Void` error handler run if the SDK runs in to an error when tokenizing.
    public func tokenizeBankAccount(
        _ bank: StaxBankAccount,
        completion: @escaping (StaxPaymentMethod) -> Void,
        error: @escaping (OmniException) -> Void
    ) {
    ...
    @available(
        *,
        deprecated,
        message: "Deprecated in favor of the `tokenizeCreditCard` function."
    )
    public func tokenize(
    ...
    /// - Parameter completion: A `(StaxPaymentMethod) -> Void` callback run after the bank has been tokenized.
    /// - Parameter error: A `(OmniException) -> Void` error handler run if the SDK runs in to an error when tokenizing.
    public func tokenizeCreditCard(
        _ card: StaxCreditCard,
        _ completion: @escaping (StaxPaymentMethod) -> Void,
        error: @escaping (OmniException) -> Void
    ) {
        guard let client = staxHttpClient else {
            return error(OmniGeneralException.uninitialized)
    ...
    @available(
        *,
        deprecated,
        message:
            "Deprecated in favor of the `takeMobileReaderTransaction` function that returns `StaxTransaction`"
    )
    ...
    /// Uses the connected Tap to Pay device to take a payment with the provided `TransactionRequest` object.
    /// - Parameter request: A `TransactionRequest` object containing the details needed to make a payment.
    /// - Parameter completion: A `(StaxTransaction) -> Void` callback run after the payment has been processed.
    /// - Parameter error: A `(OmniException) -> Void` error handler run if the SDK runs in to an error when processing.
    /// - Note: `Omni` should be assigned a `SignatureProviding` object by the time this transaction is called.
    /// - Note: This method is only available on iOS 17.4 and later due to Tap to Pay requirements.
    @available(iOS 17.4, *)
    ...
    @available(
        *,
        deprecated,
        message:
            "Deprecated in favor of the `capturePreAuthTransaction` function."
    )
    public func capturePreauthTransaction(
        ...
        let job = CapturePreauthTransaction(
    ...
    public func capturePreAuthTransaction(
        ...
        let job = CapturePreAuthTransactionJob(
            transactionId: transactionId,
            client: client,
            amount: amount
        )
    ...
    @available(
        *,
        deprecated,
        message:
            "Deprecated in favor of the `voidTransaction` function with `StaxTransaction` as return type."
    )
    ...
    public func cancelMobileReaderTransaction(
        ...
        Task {
            ...
            case .success(let result):
                self.preferredQueue.async { completion(result) }
    ...
    ///   - error: Receives any errors that happened while attempting the operation
    public func refundMobileReaderTransaction(
    ...
    public func getAvailableReaders(
    ...
    public func getConnectedReader(
        completion: @escaping (MobileReader?) -> Void,
        error: @escaping (OmniException) -> Void
    ) {
        guard isInitialized else {
    ...
    @available(
        *,
        deprecated,
        message: "Please use the connect method that provides error handling"
    )
    public func connect(
        reader: MobileReader,
        completion: @escaping (MobileReader) -> Void,
        ...
        let job = ConnectToMobileReaderJob(
            reader: reader,
            connectionStatusDelegate: mobileReaderConnectionUpdateDelegate
        )
        Task {
            let result = await job.start()
            switch result {
    ...
    /// Attempts to connect to an available Tap to Pay device on this iPhone.
    /// - Parameter completion: A `() -> Void` callback invoked upon a successful connection to Tap to Pay.
    /// - Parameter error: A `(OmniException) -> Void` error handler invoked if the SDK encounters an error while connecting.
    /// - Note: Requires iOS 17.4 or later and a compatible device with Tap to Pay set up.
    @available(iOS 17.4, *)
    public func connectToTap(
        ...
        let job = ConnectToTapJob(
            connectionStatusDelegate: tapConnectionUpdateDelegate
        )
        Task {
            let result = await job.start()
            switch result {
    ...
    ///   - error: A block to call if this operation fails
    public func getMobileReaderTransactions(
        completion: @escaping ([Transaction]) -> Void,
        error: @escaping (OmniException) -> Void
    ) {
        ...
        transactionRepository.getList(
            completion: ({ paginatedTransactions in
                guard let transactions = paginatedTransactions.data else {
                    error(
                        OmniNetworkingException.couldNotGetPaginatedTransactions
                    )
                    return
                }
    ...
    internal func getMobileReaderAuthDetails(_ apiKey: String) async throws
        -> MobileReaderDetails?
    {
        guard let client = staxHttpClient else {
            throw OmniGeneralException.uninitialized
    ...
    public func getInvoice(
        invoiceID: String,
        completion: @escaping (StaxInvoice) -> Void,
        error: @escaping (Error) -> Void
    ) {
        Task {
            do {
                let invoice = try await staxInvoiceRepository.getInvoice(
                    id: invoiceID
                )
                completion(invoice)
            } catch let err {
    ...
    public func createInvoice(
        ...
        Task {
            do {
                let invoice = try await staxInvoiceRepository.createInvoice(
                    request
                )
                completion(invoice)
            } catch let err {
    ...
    public func updateInvoice(
        ...
        Task {
            do {
                let updatedInvoice =
                    try await staxInvoiceRepository.updateInvoice(
                        id: id,
                        update: update
                    )
    ...
    public func getCatalogItem(
        id: String,
        completion: @escaping (StaxCatalogItem) -> Void,
        error: @escaping (Error) -> Void
    ) {
        Task {
            do {
                let item = try await staxCatalogRepository.getCatalogItem(
                    id: id
                )
                completion(item)
            } catch let err {
    ...
    public func createCatalogItem(
        ...
        Task {
            do {
                let createdItem =
                    try await staxCatalogRepository.createCatalogItem(
                        item: item
                    )
    ...
    public func updateCatalogItem(
        ...
        Task {
            do {
                let updatedItem =
                    try await staxCatalogRepository.updateCatalogItem(
                        id: id,
                        update: update
                    )
    ...
    ///   - completion: Receives the updated Catalog Item
    ///   - error: A block to call if this operation fails
    public func incrementCatalogItemStock(
        id: String,
        amount: Int,
        completion: @escaping (StaxCatalogItem) -> Void,
        error: @escaping (Error) -> Void
        ...
        Task {
            do {
                let updatedItem =
                    try await staxCatalogRepository.incrementCatalogItemStock(
                        id: id,
                        amount: amount
                    )
                completion(updatedItem)
    ...
    ///   - completion: Receives the updated Catalog Item
    ///   - error: A block to call if this operation fails
    public func decrementCatalogItemStock(
        id: String,
        amount: Int,
        completion: @escaping (StaxCatalogItem) -> Void,
        error: @escaping (Error) -> Void
        ...
        Task {
            do {
                let updatedItem =
                    try await staxCatalogRepository.decrementCatalogItemStock(
                        id: id,
                        amount: amount
                    )
                completion(updatedItem)
    ...
    public func getCustomer(
        id: String,
        completion: @escaping (StaxCustomer) -> Void,
        error: @escaping (Error) -> Void
    ) {
        Task {
            do {
                let customer = try await staxCustomerRepository.getCustomer(
                    id: id
                )
                completion(customer)
            } catch let err {
    ...
    ///   - error: A block to call if this operation fails
    public func getPaymentMethodsForCustomer(
        id: String,
        completion: @escaping ([StaxPaymentMethod]) -> Void,
        error: @escaping (Error) -> Void
    ) {
        Task {
            do {
                let paymentMethods =
                    try await staxCustomerRepository
                    .getPaymentMethodsForCustomer(id: id)
    ...
    public func createCustomer(
        ...
        Task {
            do {
                let createdCustomer =
                    try await staxCustomerRepository.createCustomer(request)
    ...
    public func updateCustomer(
        ...
        Task {
            do {
                let updatedCustomer =
                    try await staxCustomerRepository.updateCustomer(
                        id: id,
                        update: update
                    )
    ...
    public func getPaymentMethod(
        ...
        Task {
            do {
                let paymentMethod =
                    try await staxPaymentMethodRepository.getPaymentMethod(
                        id: id
                    )
    ...
    ///   - error: A block to call if this operation fails
    public func getPaymentMethodSurcharge(
        id: String,
        dollars: Double,
        completion: @escaping (StaxSurchargeReview) -> Void,
        error: @escaping (Error) -> Void
    ) {
        Task {
            do {
                let surcharge =
                    try await staxPaymentMethodRepository
                    .getPaymentMethodSurcharge(id: id, dollars: dollars)
    ...
    public func updatePaymentMethod(
        id: String,
        update: StaxPaymentMethod.Update,
        completion: @escaping (StaxPaymentMethod) -> Void,
        error: @escaping (Error) -> Void
    ) {
        Task {
            do {
                let updatedPaymentMethod =
                    try await staxPaymentMethodRepository.updatePaymentMethod(
                        id: id,
                        update: update
                    )
    ...
    ///   - completion:Called upon success
    public func deletePaymentMethod(
        id: String,
        completion: @escaping () -> Void,
        error: @escaping (Error) -> Void
    ) {
        Task {
            do {
                try await staxPaymentMethodRepository.deletePaymentMethod(
                    id: id
                )
                completion()
            } catch let err {
