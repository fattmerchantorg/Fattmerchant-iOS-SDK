...
-(void) showAlertView:(NSString*)title message:(NSString*)message; //!<When a popup is needed, passing strings will have the app have one display with the title and message supplied
//!< @param title The title of the popup
//!< @param message The message to display inside of the popup
...
* Set BLE Delay
*
Setd the BLE Conneciton Delay. Default 0.5 seconds. Minimum Value Allowed 0.5 seconds.

*/
+(void) setBLEDelay:(float)delay;
...
/**
 * Sends a PK Update with File Name
 *
 Starts a device firmware upatate using the provided path to the .pk file.
 * Update proceeds on a background thread.
 * Use protocol UpdateStatus to monitor progress.
 * Uses the File's Name to run checks to make sure it is appropriate for the device to avoid using one for another device.
 * Keep device attached and do interrupt process until PK_STATUS_FAILED or PK_STATUS_COMPLETED has been received
 *
 @param pkFile The .pk file
 @param fileName The name of the pk file to be checked
 ...
 */
-(RETURN_CODE) sendPKUpdate:(NSData*)pkFile fileName:(NSString*)fileName;
